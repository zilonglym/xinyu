 ...
 <form  id="subForm" action="${ctx}/store/shipOrder/submitDeliverOrder/${order.id}" method="post">
		...
		<tr>
			...
			<select name="logist">
				<c:forEach items="${logistList}" var="obj">
					...
					<c:if test="${obj.value == order.expressCompany}">
						selected='selected'
					</c:if>>${obj.description}</option>
				</c:forEach>
			</select>
			</td>
			<td>运单号:</td>
			<td><input type="text" name="expressCode" value="${order.expressOrderno}" readonly="readonly"/></td>
	...
	<table id="contentTable" class="table table-condensed alert">
		...
		<tr>
				...
				<input name="numTemp" type="hidden" value = "${detail.num - detail.actualnum}"/><input name="num" type="text" value = "${detail.num}"/></td>
