...
group.initTable=function(){
	$('#tb_group').datagrid({
	    ...
	    columns:[[
	        ...
	        {field:'level',title:'优先级',width:60},
...
function initDetailTable(rowData){
	if(rowData){
		$('#tb_groupDetail').datagrid({
		    url:ctx+'/itemGroup/detail/listData',
		    height:150,
		    singleSelect:true,
		    rownumbers:true,
		    queryParams:{
...
group.close = function(id){
	$.messager.confirm('提示信息', '是否继续?', function(r) {
		if (id) {
			$.post(ctx + "/itemGroup/closeItemGroupState", {
				id:id
			}, function(data) {
...
group.open = function(id){
	$.messager.confirm('提示信息', '是否继续?', function(r) {
		if (id) {
			$.post(ctx + "/itemGroup/openItemGroupState", {
				id:id
			}, function(data) {
				$.messager.alert('提示', data.msg);
				$('#tb_group').datagrid('load',{
	...
	$('#dialog').dialog({
	    ...
	    buttons: [{
                    ...
                    handler:function(){
                     ...
                     json=json+"level:'"+$("#priority").combobox("getValue")+"',";
                     ...
                     $.post(ctx+"/itemGroup/save",{json:json,rows:dateStr},function(data){
                    		 $.messager.alert("提示",data.msg);
                    		 $('#dialog').window('close');
                    		 $('#tb_group').datagrid('load',{
                    			userId:$('#userId').combobox('getValue'),
...
function batchAppend(){
	...
	$('#item').dialog({
		...
		closed:false,
		cache:false,
		href:ctx+'/itemGroup/f7Item/list',
		queryParams:{
			userId:userId
		},
		modal:true,
...
function initOrderItem(id){ 
	var url = ctx+'/itemGroup/detail/listData';
...
function initNewAddTable(dataStr){ 
	var url = ctx+'/itemGroup/newItemList/listData';
	var editRow = undefined;
	$('#tb_itemDetail').datagrid({
	    url:url,
	    height:600,
