...
items.updateItemType=function(id){
	$('#dialog1').dialog({
	    ...
	    width: 400,
	    height: 200,
	    href: ctx+'/item/tof7ItemType?id='+id,
	    closed: false,
	    cache: false,
	    modal: true,
	    buttons: [{
...
items.initTable=function(){
	$('#contentTable').datagrid({
	    ...
	    remoteSort:false,
	    columns:[[
	        {field:'itemName',title:'商品名称',width:260,sortable:true,
				  sorter:function(a,b){
						return (a>b?1:-1);
					}  
	        },
	        {field:'itemCode',title:'商品编码',width:120,sortable:true,
				  sorter:function(a,b){
						return (a>b?1:-1);
					}  
	        },
	        {field:'sku',title:'商品SKU',width:120,sortable:true,
				  sorter:function(a,b){
						return (a>b?1:-1);
					}  
	        },
	        ...
	        {field:'barCode',title:'商品条码',width:120,sortable:true,
				  sorter:function(a,b){
						return (a>b?1:-1);
					}  
	        },
	        {field:'weight',title:'净重量',width:80,sortable:true,
				  sorter:function(a,b){
						return (a>b?1:-1);
					}  
	        },
	        {field:'packageWeight',title:'打包重量',width:80,sortable:true,
				  sorter:function(a,b){
						return (a>b?1:-1);
					}  
	        },
	        {field:'userName',title:'所属商家',width:150,sortable:true,
				  sorter:function(a,b){
						return (a>b?1:-1);
					}  
	        },
...
items.printBarCode=function(id){
	 $.messager.prompt('条码打印', '请输入条码打印数量', function(r){
                if (r){
                    window.open(ctx+"/import/toPoint?itemId="+id+"&count="+r);
                }
            });
};
