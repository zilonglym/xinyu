...
public class DataView extends Frame {
   ...
   // private String url="http://admin.wlpost.com/check/checkTrade_new";
    ...
    public static String url="http://175.6.27.205:8080/xinyu-check/";

//    public static String url="http://127.0.0.1:8080/xinyu-check/";
                    ...
                    try {
                       ...
                       // 自定义解析过程，你在实际使用过程中可以按照自己的需求在接收到数据后对数据进行解析
                            if (data == null || data.length < 1) {    //检查数据是否读取正确
                            	logger.error(data);
                            	logger.error("读取数据过程中未获取到有效数据！请检查设备或程序！");
                                ...
                                logger.error(dataOriginal);
                                logger.error(commStr.toString());
                                ...
                                if (commStr.toString().indexOf("*")>=0 && commStr.toString().indexOf("@")>=0) {    //当数据的第一个字符是*号，并且字符串有还有@ 时表示数据接收完成，开始解析                            
                                	logger.error("[["+commStr+"]]");
                                	resultAry=commStr.toString().split("@");//拿出第一组数据
                                	dataValid=resultAry[0].substring(1);//去掉数据前面的符号，组成完整的一组数据
                                	String tempStr=commStr.toString().replace("*"+dataValid+"@", "");//在原有的数据中间去掉这一组数据
                                	tempStr=tempStr.replaceAll("\\*", "");
                                	tempStr=tempStr.replaceAll("\\\n", "");
                                	tempStr=tempStr.replaceAll("\\\r", "");
                                	commStr=new StringBuffer("");
                                	commStr.append(tempStr);
                                    ...
                                    if (elements == null || elements.length < 1) {    //检查数据是否解析正确
                                        ...
                                        try {
                                            ...
                                            //取得数据成功，调用后台数据
	                                          // String tempUrl=url+"check/checkTrade_new?stock=1&orderCode="+elements[1]+"&barCode="+elements[0]+"&persinId="+Client.personId;
	                                           String tempUrl=url+"check/orderCheck?source=1&stock=1&orderCode="+elements[1]+"&barCode="+elements[0]+"&persinId="+Client.personId;
	                                           int k=0;
	                                           String resultStr= HttpClientUtils.httpPost(tempUrl, "");
//	                                           String result=new String(resultStr.getBytes(),"gbk");
	                                           logger.error("此次验货用时长:"+(new Date().getTime()-date.getTime()));
	                                           ...
	                                           if(code!=null && code.equals("200")){
	                                        	   ...
	                                        	   status.setBackground(Color.red);
	                                        	   logger.error("拦截出库");
	                                        	   logger.error(tempUrl);
	                                        	   logger.error(""+elements[0]+"|"+elements[1]);
	                                        	   ...
	                                        	   logger.error(tempUrl);
	                                        	   logger.error(""+elements[0]+"|"+elements[1]);
	                                        	   msgList.add(orderCode.getText()+" "+message.substring(message.length())+"  \r\n");
	                                        	   k=1;
	                                           }
	                                           if(k==1){
                                        	...
                                        	logger.error("Data Clean commStr! 数据解析过程出错，更新界面数据失败！请检查设备或程序！");
                                        }
                                    }    
                                }
                            }                            
                    	...
                    	commStr=new StringBuffer("");
                    	logger.error(e.getMessage());
                    	logger.error("Data Clean commStr! ！"+e.getLocalizedMessage()+"||||"+e);
...
//                        System.exit(0);    //发生读取错误时显示错误信息后退出系统
    ...
    public static void testhttpPost(){
    	 ...
    	 String resultStr= HttpClientUtils.httpPost(url+"check/orderCheck?1=1", "");
    	 System.err.println(resultStr);
    }
    
    public static void testSound() throws Exception{
    	  InputStream successIn =null; 
    	  AudioStream successAs =null;
    	  successIn =new FileInputStream ("D:\\sound\\success.wav"); // 打 开 一 个 声 音 文 件 流 作 为 输 入
     	  successAs =new AudioStream (successIn); // 用 输 入 流 创 建 一 个AudioStream 对 象 
//     	  AudioData ad=successAs.getData();  
//     	  ContinuousAudioDataStream cads = new ContinuousAudioDataStream(ad);
     	  for(int i=0;i<10;i++){
     		  AudioPlayer.player.start (successAs);
     	  }
    }
    
    public static void testJson(){
    	 String tempUrl=url+"check/orderCheck?source=1&stock=1&orderCode=6921969804369&barCode=3956720663054&persinId=1";
