...
public class TradeService {
	...
	public Map<String,Object>  reSetAudit(Long  tradeId,Long centroId,Long  userId,Long operator){
		...
		String orderStatus = shipOrder.getStatus();
		...
		if(shipOrder!=null && shipOrder.getSource()!=null && shipOrder.getSource().equals(Trade.SourceFlag.SourceFlag_QM)){
			// 还原订单状态
			if (shipOrder.getExpressOrderno()==null) {
				tradeDao.updateTradeStatus(tradeId, Trade.Status.TRADE_WAIT_CENTRO_AUDIT);
			}else {	
				Trade trade = this.tradeDao.getTrade(tradeId);
				String newOtherOrderNo = String.valueOf(trade.getOtherOrderNo())+"0";
				tradeDao.updateTradeStatusAndOtherNo(tradeId, Trade.Status.TRADE_WAIT_CENTRO_AUDIT,Long.parseLong(newOtherOrderNo));
			}
		}else{
			tradeDao.updateTradeStatus(tradeId, Trade.Status.TRADE_WAIT_CENTRO_AUDIT);
		}
		...
		for(int i = 0,  size  = detailList.size();i<size;i++   ){
			...
			if(existOrderNo>0){//清空订单库存记录    否则无法扫描出库
				...
				if ("WMS_FINISH".equals(orderStatus)) {
					this.inventoryService.updateUserNum(centroId, itemId, Accounts.CODE_SALEABLE, detail.getNum());
	...
	public  void createTradesFromTop(String[] tids) throws NumberFormatException, ApiException {
		...
		for (String tid : tids) {
			try {
				...
				MessageContextHelper.append("TID:"+tid+" 创建失败：" + e.getMessage());
			}
		}
		int success = 0;
...
//		if (tradeId == null) {
			// 状态等待物流通审核
			trade.setOtherOrderNo(UUID.randomUUID().toString());
		...
		try{
			...
			if(shipOrder==null && trade.getTradeFrom()!=null && !trade.getTradeFrom().equals(SourceFlag.SourceFlag_QM)
				...
				logger.debug("导入订单审核成功:"+shipOrder.getId()+",所花时间:"+tt);
				 System.err.println("2:"+shipOrder.getItems());
				return shipOrderService.getShipOrder(shipOrder.getId());
			}else{
				/**
				 * 奇门的单据审核主要完成功能有
...
//			itemBuf.append("【").append(total).append("KG】 |");
	...
	public void deleteTrade(Long tradeId,int userId) {
		logger.debug("奇门订单删除操作开始！tradeId："+tradeId);
		...
		try{
			...
			List<ShipOrderDetail> detailList = order.getDetails();
			/**
			 * 订单删除
			 * userNum返回库存
			 * */
			 ...
			 * @modify 2017-08-24
			 * @author fufangjue
			 * 订单删除非取消和非作废状态的订单修改库存
			 * 否则直接删除订单，不再作修改库存操作
			 * */
			if(order.getSource().equals(Trade.SourceFlag.SourceFlag_QM)&&status!="WMS_DELETE"&&status!="WMS_CANCEL"){
				for(int i=0;detailList!=null && i<detailList.size();i++){
					logger.debug("奇门订单删除,库存修改开始！itemId:"+detailList.get(i).getItem().getId());
					ShipOrderDetail detail=detailList.get(i);
					String  uid = String.valueOf(order.getCreateUser().getId());
//					this.qmInventoryService.addInventory(1L, Long.parseLong(uid), detail.getItem().getId(), detail.getNum(), Accounts.CODE_SALEABLE, "订单删除", String.valueOf(order.getId()));
		...
		/*	*/
			...
			logger.debug("奇门订单删除操作结束！tradeId："+tradeId+" 耗时："+(end-start));
		}catch(Exception e){
			e.printStackTrace();
	...
	public void invalidTrade(long tradeId,Long userId){
		...
		if(order.getSource().equals(Trade.SourceFlag.SourceFlag_QM)){
			 ...
			 * 其他状态：有userNum返回库存
			 * */
			if("WMS_FINISH".equals(orderState)){
				for(int i=0;detailList!=null && i<detailList.size();i++){
					ShipOrderDetail detail=detailList.get(i);
					this.qmInventoryService.addInventory(1L, order.getCreateUser().getId(), detail.getItem().getId(), detail.getNum(), Accounts.CODE_SALEABLE, "订单作废", order.getExpressOrderno());
				}
			}else{
				for(int i=0;detailList!=null && i<detailList.size();i++){
					ShipOrderDetail detail=detailList.get(i);
	...
	/**
	 * 根据shipOrder创建trade（新）
	 * */
	@Transactional(readOnly = false)
	public Trade createTradePrint(Trade trade, Long tid, String tbNumber) {
		trade.setStatus(Trade.Status.TRADE_WAIT_CENTRO_AUDIT);
		trade.setTradeFrom(Trade.SourceFlag.SourceFlag_ERP+" | "+tbNumber);
		trade.setOtherOrderNo(tid.toString());
		tradeJpaDao.save(trade);
		List<TradeOrder> orders = trade.getOrders();
		if (CollectionUtils.isNotEmpty(orders)) {
			for (TradeOrder tradeOrder : orders) {
