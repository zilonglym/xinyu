...
public class TradeAdapter {
	...
	public Trade adapter(com.taobao.api.domain.Trade tTrade) throws ApiException {
		...
		trade.setSellerFlag(String.valueOf(tTrade.getSellerFlag()));
		...
		if (tTrade.getHasBuyerMessage() != null && tTrade.getHasBuyerMessage() == true) {
			...
			tTrade.setBuyerMemo(trade.getBuyerMemo()+topTrade.getBuyerMemo());
			tTrade.setBuyerMessage(trade.getBuyerMessage()+topTrade.getBuyerMessage());
			tTrade.setSellerMemo(trade.getSellerMemo()+topTrade.getSellerMemo());
		}else{
		...
		if (CollectionUtils.isEmpty(orders)) {
			...
			order.setTitle(item.getTitle()+Trade.titleAndCodeSplit+item.getOuterId());
			...
			for (Order order : orders) {
				...
				com.graby.store.entity.Item item=this.itemService.getItem(tradeOrder.getItem().getId());
				if(item!=null ){
					tradeOrder.setItem(item);
					weight+=item.getPackageWeight()*order.getNum();
				}
