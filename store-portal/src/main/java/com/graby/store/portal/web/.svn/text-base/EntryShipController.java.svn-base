...
public class EntryShipController {
	...
	public String createEntryForm(Model model) {
		...
		model.addAttribute("centroList",centroList);
	...
	public String createEntry(ShipOrder order, Model model) {
		try {
			System.out.println("start:创建入库单据:"+order+"::"+shipOrderService);
			shipOrderService.saveEntryOrder(order);
			model.addAttribute("order", order);
			System.out.println("end:创建入库单据:"+order);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String item(
		...
		model.addAttribute("order", order);
		Centro centro=this.centroService.findCentroById(String.valueOf(order.getCentroId()));
	...
	public String itemDetail(@PathVariable("id") Long orderId, Model model) {
		...
		params.put("shipOrderId", order.getId());
		List<ShipOrderDetail> detail=this.shipOrderService.shipOrderDetailbyList(params);
		for(int i=0;detail!=null && i<detail.size();i++){
			ShipOrderDetail obj=detail.get(i);
			Item item=this.itemService.getItem(obj.getItem().getId());
			obj.setItem(item);
		}
