...
public class BaseDaoImpl extends SqlSessionDaoSupport implements BaseDao {
	...
	public Object execute(SqlSessionCallback sqlSessionCallback) {
		SqlSession sqlSession = SqlSessionUtils.getSqlSession(sqlSessionFactory, ExecutorType.BATCH, new MyBatisExceptionTranslator(sqlSessionFactory.getConfiguration()
	...
	public <T> Pagination<T> selectList(String statement, Object parameter, Pagination<T> page) {
		if (page == null) {
			...
			if (count == 0) {
				...
				List<T> items = getCustomSqlSession().selectList(statement, parameter, new RowBounds(page.getCurrentPageStartIndex(), page.getPageSize()));
				page.setItems(items);
	...
	public int selectCount(String statement, Object parameter) {
		...
		return (Integer) getCustomSqlSession().selectOne(statement, new PaginationInterceptor.CountParameter(parameter));
	}
				...
				public Object invoke(final MethodInvocation invocation) throws Throwable {
					...
					try {
						...
						/*if (transactionLocal.get() != null) {
							...
							// batch 注解在transaction标签之后由事物拦截器去关闭所有连接
						} else {*/
